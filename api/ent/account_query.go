...
type AccountQuery struct {
	...
	withOutgoingTransactions *TransactionQuery
	withIncomingTransactions *TransactionQuery
...
// QueryOutgoingTransactions chains the current query on the "outgoing_transactions" edge.
func (aq *AccountQuery) QueryOutgoingTransactions() *TransactionQuery {
...
// QueryIncomingTransactions chains the current query on the "incoming_transactions" edge.
func (aq *AccountQuery) QueryIncomingTransactions() *TransactionQuery {
	query := (&TransactionClient{config: aq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := aq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (aq *AccountQuery) Clone() *AccountQuery {
	...
	return &AccountQuery{
		...
		withOutgoingTransactions: aq.withOutgoingTransactions.Clone(),
		withIncomingTransactions: aq.withIncomingTransactions.Clone(),
...
// WithOutgoingTransactions tells the query-builder to eager-load the nodes that are connected to
// the "outgoing_transactions" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithOutgoingTransactions(opts ...func(*TransactionQuery)) *AccountQuery {
	query := (&TransactionClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withOutgoingTransactions = query
	return aq
}

// WithIncomingTransactions tells the query-builder to eager-load the nodes that are connected to
// the "incoming_transactions" edge. The optional arguments are used to configure the query builder of the edge.
func (aq *AccountQuery) WithIncomingTransactions(opts ...func(*TransactionQuery)) *AccountQuery {
	query := (&TransactionClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	aq.withIncomingTransactions = query
	return aq
}
...
func (aq *AccountQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Account, error) {
	var (
		...
		_spec       = aq.querySpec()
		loadedTypes = [2]bool{
			aq.withOutgoingTransactions != nil,
			aq.withIncomingTransactions != nil,
		}
	)
	...
	if query := aq.withOutgoingTransactions; query != nil {
		if err := aq.loadOutgoingTransactions(ctx, query, nodes,
			func(n *Account) { n.Edges.OutgoingTransactions = []*Transaction{} },
			func(n *Account, e *Transaction) {
				n.Edges.OutgoingTransactions = append(n.Edges.OutgoingTransactions, e)
			}); err != nil {
			return nil, err
		}
	}
	if query := aq.withIncomingTransactions; query != nil {
		if err := aq.loadIncomingTransactions(ctx, query, nodes,
			func(n *Account) { n.Edges.IncomingTransactions = []*Transaction{} },
			func(n *Account, e *Transaction) {
				n.Edges.IncomingTransactions = append(n.Edges.IncomingTransactions, e)
			}); err != nil {
			return nil, err
		}
	}
...
func (aq *AccountQuery) loadOutgoingTransactions(ctx context.Context, query *TransactionQuery, nodes []*Account, init func(*Account), assign func(*Account, *Transaction)) error {
	...
	query.Where(predicate.Transaction(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.OutgoingTransactionsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.account_outgoing_transactions
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_outgoing_transactions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_outgoing_transactions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (aq *AccountQuery) loadIncomingTransactions(ctx context.Context, query *TransactionQuery, nodes []*Account, init func(*Account), assign func(*Account, *Transaction)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Account)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Transaction(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(account.IncomingTransactionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.account_incoming_transactions
		if fk == nil {
			return fmt.Errorf(`foreign-key "account_incoming_transactions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "account_incoming_transactions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
