...
func (tq *TransactionQuery) loadFromAccount(ctx context.Context, query *AccountQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Account)) error {
	...
	for i := range nodes {
		if nodes[i].account_outgoing_transactions == nil {
			continue
		}
		fk := *nodes[i].account_outgoing_transactions
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_outgoing_transactions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (tq *TransactionQuery) loadToAccount(ctx context.Context, query *AccountQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Account)) error {
	...
	nodeids := make(map[int][]*Transaction)
	for i := range nodes {
		if nodes[i].account_incoming_transactions == nil {
			continue
		}
		fk := *nodes[i].account_incoming_transactions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "account_incoming_transactions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
